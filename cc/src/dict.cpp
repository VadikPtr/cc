...
bool DictV::init(const DictV& other) {
  init(other.vtable_);
  ...
  for (size_t i = 0; i < bucket_count(); ++i) {
    ...
    if (metadata_[i] != g_vt_empty) {
      vtable_->copy_key(get_bucket_key(get_bucket(i)),
                        other.get_bucket_key(other.get_bucket(i)));
      vtable_->copy_value(get_bucket_value(get_bucket(i)),
                          other.get_bucket_value(other.get_bucket(i)));
    }
  }
  return true;
}

void DictV::init(DictV&& other) {
...
void DictV::init(const DictVTable* a_vtable) {
  key_count_    = 0;
  buckets_mask_ = 0;
  buckets_      = nullptr;
...
void DictV::clear() {
  ...
  for (size_t i = 0; i < bucket_count(); ++i) {
    if (metadata_[i] != g_vt_empty) {
      vtable_->destroy_key(get_bucket_key(get_bucket(i)));
      vtable_->destroy_value(get_bucket_value(get_bucket(i)));
    }
...
DictVItr DictV::insert(void* key, void* value) {
  while (true) {
    DictVItr itr = insert_raw(key, value, false, true);
    if (!itr.is_end() ||
...
DictVItr DictV::get(void* key) {
  ...
  while (true) {
    if ((metadata_[bucket] & g_vt_hash_frag_mask) == hashfrag &&
        vtable_->equals_key(get_bucket_key(get_bucket(bucket)), key)) {
      DictVItr itr;
...
bool DictV::erase_itr_raw(DictVItr itr) {
  ...
  if (itr.home_bucket_ == SIZE_MAX) {
    if (metadata_[itr_bucket] & g_vt_in_home_bucket_mask) {
      ...
      itr.home_bucket_ =
          vtable_->hash(get_bucket_key(get_bucket(itr_bucket))) & buckets_mask_;
    }
  }
  ...
  while (true) {
    ...
    if ((metadata_[bucket] & g_vt_displacement_mask) == g_vt_displacement_mask) {
      vtable_->move_bucket(get_bucket(itr_bucket), get_bucket(bucket));
...
bool DictV::erase(void* key) {
  DictVItr itr = get(key);
  if (itr.is_end()) {
    return false;
  }
  erase_itr_raw(itr);
...
void DictV::erase(const DictVItr& it) {
  assert(!it.is_end());
  erase_itr_raw(it);
}

DictVItr DictV::begin() const {
  if (!key_count_) {
    return end();
  }
  DictVItr itr;
...
size_t DictV::total_alloc_size() const {
  return metadata_offset() + (buckets_mask_ + 1 + 4) * sizeof(u16);
}

size_t DictV::size() const {
  return key_count_;
}

size_t DictV::bucket_count() const {
  return buckets_mask_ + (bool)buckets_mask_;
}

size_t DictV::metadata_offset() const {
  return (((buckets_mask_ + 1) * vtable_->bucket_size + sizeof(u16) - 1) / sizeof(u16)) *
         sizeof(u16);
}

u8* DictV::get_bucket(size_t idx) const {
  return static_cast<u8*>(buckets_) + idx * vtable_->bucket_size;
}

u8* DictV::get_bucket_key(u8* b) const {
  return b + vtable_->key_offset;
}

u8* DictV::get_bucket_value(u8* b) const {
  return b + vtable_->value_offset;
}

bool DictV::find_first_empty(size_t home_bucket, size_t& empty, u16& displacement) const {
  displacement               = 1;
...
size_t DictV::find_insert_location_in_chain(size_t home_bucket,
...
DictVItr DictV::insert_raw(void* key, void* val, bool unique, bool replace) {
  u64    hash        = vtable_->hash(key);
  u16    hashfrag    = vt_hashfrag(hash);
  size_t home_bucket = hash & buckets_mask_;
  ...
  if (!(metadata_[home_bucket] & g_vt_in_home_bucket_mask)) {
    ...
    vtable_->move_key(get_bucket_key(get_bucket(home_bucket)), key);
    vtable_->move_value(get_bucket_value(get_bucket(home_bucket)), val);
  ...
  if (!unique) {
    ...
    while (true) {
      if ((metadata_[bucket] & g_vt_hash_frag_mask) == hashfrag &&
          vtable_->equals_key(get_bucket_key(get_bucket(bucket)), key)) {
        if (replace) {
          vtable_->destroy_key(get_bucket_key(get_bucket(bucket)));
          vtable_->move_key(get_bucket_key(get_bucket(bucket)), key);
          vtable_->destroy_value(get_bucket_value(get_bucket(bucket)));
          vtable_->move_value(get_bucket_value(get_bucket(bucket)), val);
        }
        DictVItr itr;
        itr.data_         = get_bucket(bucket);
        itr.metadatum_    = metadata_ + bucket;
  ...
  vtable_->move_key(get_bucket_key(get_bucket(empty)), key);
  vtable_->move_value(get_bucket_value(get_bucket(empty)), val);
...
bool DictV::evict(size_t bucket) {
  size_t home_bucket = vtable_->hash(get_bucket_key(get_bucket(bucket))) & buckets_mask_;
  ...
  vtable_->move_bucket(get_bucket(empty), get_bucket(bucket));
...
bool DictV::rehash(size_t bucket_count) {
  while (true) {
    DictV new_table;
    ...
    for (size_t bucket = 0; bucket < this->bucket_count(); ++bucket) {
      if (metadata_[bucket] != g_vt_empty) {
        DictVItr itr =
            new_table.insert_raw(get_bucket_key(get_bucket(bucket)),
                                 get_bucket_value(get_bucket(bucket)), true, false);
        if (itr.is_end()) {
          break;
        }
      }
...
bool DictV::reserve(size_t size) {
