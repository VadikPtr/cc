...
int getopt_create_context(getopt_context_t* ctx, int argc, const char** argv,
  ...
  while (!(opt->name == 0x0 && opt->name_short == 0)) {
    if (opt->value == '!' || opt->value == '?' || opt->value == '+' || opt->value == -1)
      return -1;

    if (opt->name) {
      if (opt->name[0] == '-') return -1;
    }
...
int getopt_next(getopt_context_t* ctx) {
  ...
  if (ctx->current_index == ctx->argc) return -1;
  ...
  if (curr_token[1] != '\0' && curr_token[1] != '-' && curr_token[2] == '\0') {
    ...
    for (; i < ctx->num_opts; i++) {
      ...
      if (opt->name_short == curr_token[1]) {
        ...
        /* if there is an value when: - current_index < argc and value in
         * argv[current_index] do not start with '-' */
        if (((ctx->current_index != ctx->argc) &&
             (ctx->argv[ctx->current_index][0] != '-')) &&
    ...
    for (; i < ctx->num_opts; i++) {
      ...
      if (str_case_cmp_len(opt->name, check_option, name_len) == 0) {
        ...
        if (getopt_opt_might_have_arg(opt)) {
          switch (*check_option) {
            case '\0': {
              if (ctx->current_index <
                  ctx->argc) /* are there more tokens that can contain the '='? */
              {
                ...
                if (next_token[0] == '=') {
                  ...
                  if (next_token[1] !=
                      '\0') { /* does this token contain the arg-value? */
                    found_arg = next_token + 1;
                  } else if (ctx->current_index < ctx->argc) {
                    found_arg = ctx->argv[ctx->current_index++]; /* next token has been
                                                                    processed aswell! */
                  }
            ...
            case '=':
              ...
              } else if (ctx->current_index <
                         ctx->argc) { /* next token has been processed aswell! */
                found_arg = ctx->argv[ctx->current_index++];
              }
              break;
            default:
...
const char* getopt_create_help_string(getopt_context_t* ctx, char* buffer,
  ...
  for (; opt_index < ctx->num_opts; ++opt_index) {
    ...
    if (opt->name_short == 0x0) {
      chars_written = str_format(buffer + buffer_pos, buffer_size - buffer_pos,
                                 "   %-32s - %s\n", long_name, opt->desc);
    } else {
      chars_written =
          str_format(buffer + buffer_pos, buffer_size - buffer_pos, "-%c %-32s - %s\n",
