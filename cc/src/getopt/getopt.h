 ...
 *   {
 *     { "help", 'h', GETOPT_OPTION_TYPE_NO_ARG, 0x0, 'h', "print help text", 0x0 },
 *    { "verbose", 'v', GETOPT_OPTION_TYPE_FLAG_SET, &verbose, 1, "verbose logging", 0x0 },
...
typedef enum getopt_option_type {
  ...
  GETOPT_OPTION_TYPE_REQUIRED_INT32,  ///< The option requires an argument and this
                                      ///< argument has to be parseable as an int
                                      ///< (--option=arg, -o arg)
  GETOPT_OPTION_TYPE_OPTIONAL_INT32,  ///< The option-argument is optional, but if it is
                                      ///< there it has to be parseable as int.
  GETOPT_OPTION_TYPE_REQUIRED_FP32,   ///< The option requires an argument and this
                                      ///< argument has to be parseable as an float
                                      ///< (--option=arg, -o arg)
  GETOPT_OPTION_TYPE_OPTIONAL_FP32,   ///< The option-argument is optional, but if it is
                                      ///< there it has to be parseable as float.
  GETOPT_OPTION_TYPE_FLAG_SET,  ///< The option is a flag and value will be set to flag
  GETOPT_OPTION_TYPE_FLAG_AND,  ///< The option is a flag and value will be and:ed with
                                ///< flag
...
typedef struct getopt_option {
  ...
  int* flag;  ///< Pointer to flag to set if option is of flag-type, set to null NULL if
              ///< option is not of flag-type.
  int value;  ///< If option is of flag-type, this value will be set/and:ed/or:ed to the
              ///< flag, else it will be returned from getopt_next when option is found.
  const char* desc;  ///< Description of option, used when generating help-text.
  const char*
      value_desc;  ///< Short description of valid values to the option, will only be used
                   ///< when generating help-text. example: "--my_option=<value_desc>"
} getopt_option_t;

/**
 * Context used while parsing options.
 * Need to be initialized by <getopt_create_context> before usage. If reused a
 * re-initialization by <getopt_create_context> is needed.
 *
...
typedef struct getopt_context {
  ...
  /**
   * Used to return values. Will point to a string that is the argument to the currently
   * parsed option. I.e. when parsing '--my-flag whoppa_doppa", this will point to "whoppa
   * doppa".
   *
   * If the option is of type GETOPT_OPTION_TYPE_OPTIONAL this will be set to NULL if
   ...
   * @note on parse-errors '!' will be returned from getopt_next() and current_opt_arg
   * will be set to the name of the option that failed to parse.
   * @note if the option is 'optional' and there was no arg, current_opt_arg will be 0x0.
   */
  union {
    /**
     * if the option is on type GETOPT_OPTION_TYPE_OPTIONAL_INT or
     * GETOPT_OPTION_TYPE_REQUIRED_INT and it parsed successfully the value will be stored
     * here. supported int formats are, decimal, hex and octal (123, 0x123, 0123)
     */
    int i32;

    /**
     * if the option is on type GETOPT_OPTION_TYPE_OPTIONAL_FP32 or
     * GETOPT_OPTION_TYPE_REQUIRED_FP32 and it parsed successfully the value will be
     * stored here.
     */
    float fp32;
  } current_value;
 ...
 * @param argv argv from "int main(int argc, char** argv)" or equal. Data need to be valid
 * during option-parsing and usage of data.
 * @param opts Pointer to array with options that should be looked for. Should end with an
 * option that is all zeroed!
 *
 * @return 0 on success, otherwise error-code.
 */
int getopt_create_context(getopt_context_t* ctx, int argc, const char** argv,
 ...
 *
 * @return '!' on error. ctx->current_opt_arg will be set to flag-name! Errors that can
 * occur, Argument missing if argument is required or Argument found when there should be
 * none.
 *         '?' if item was an unrecognized option, ctx->current_opt_arg will be set to
 * item!
 *         '+' if item was no option, ctx->current_opt_arg will be set to item!
 *         '0' if the opt was a flag and it was set. ctx->current_opt_arg will be set to
 * flag-name! the value stored is value in the found option. -1 no more options to parse!
 */
 ...
 */
const char* getopt_create_help_string(getopt_context_t* ctx, char* buffer,
                                      size_t buffer_size);
